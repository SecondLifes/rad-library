...
- `numeric-locale.md` — `StrToFloat`/`StrToCurr` and the locale decimal
  separator: the silent "comma vs dot" zero-default bug (Turkish locale
  uses decimal comma!), explicit `TFormatSettings`, Invariant for
  machine-facing formats (JSON/SQL/CSV).
