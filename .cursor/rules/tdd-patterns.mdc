...
All test projects and units live under `src/test/`. A test filename repeats
the source filename and appends `.test` (`help.date.test.pas`). Every
approved public method needs success, boundary and error-path tests.
Compile-check Win32/Win64 and applicable VCL/FMX packages. Keep vendor tests
separate and benchmark every performance claim.
