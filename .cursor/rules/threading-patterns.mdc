...
Core/helper code should be reentrant and thread-safe where practical. Do not
use mutable global state. Protect shared caches and define their lifetime.
VCL/FMX UI work is main-thread-only; marshal background results. Document
non-thread-safe APIs and vendor-specific concurrency limits explicitly.
...
## Closure Capture in Loops (Trap)

Delphi anonymous methods capture **variables, not values** — and locals
are method-scoped, not block-scoped. Every closure created in a loop
shares the SAME variable, including a "local copy" declared in the loop
body:

```pascal
//❌ YANLIŞ — tüm task'lar AYNI LId değişkenini paylaşır;
//çalıştıklarında çoğu son değeri görür
for LId in LIds do
  TTask.Run(procedure begin ProcessId(LId); end);

//❌ YİNE YANLIŞ — LIdLocal da metot kapsamındadır, kopya işe yaramaz
for LId in LIds do
begin
  LIdLocal := LId;
  TTask.Run(procedure begin ProcessId(LIdLocal); end);
end;

//✅ DOĞRU — her çağrı yeni bir aktivasyon kaydı oluşturur;
//parametre değeri closure'a güvenle bağlanır
function CaptureProcess(AId: Integer): TProc;
begin
  Result := procedure
            begin
              ProcessId(AId);
...
for LId in LIds do
  TTask.Run(CaptureProcess(LId));
```

The factory-function pattern is the only reliable per-iteration capture
in Delphi; C#/JS-style block scoping does not exist here.
