...
dcc32 -B -Q -$R+ -$Q+ -$S+ -NSSystem;System.Win;Winapi;Vcl;Data;Xml;Web;Soap ^
  -U"..\..\..\core;%MORMOT%\core;%MORMOT%" -I"%MORMOT%;%MORMOT%\core" ^
  -E"%~dp0bin" -N0"%~dp0dcu" "CacheLoadTest.dpr" > "%LOG%" 2>&1
set ERR=%ERRORLEVEL%
if %ERR% NEQ 0 ( type "%LOG%" & echo === DERLEME BASARISIZ === & exit /b %ERR% )

"%~dp0bin\CacheLoadTest.exe" >> "%LOG%" 2>&1
set ERR=%ERRORLEVEL%
type "%LOG%"
exit /b %ERR%
